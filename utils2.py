...
class Image_data:
    ...
    def preprocess(self):
        for idx, label in enumerate(self.label_list) :
            ...
            label_one_hot = [[idx]] * len(image_list) 
...
def imsave(images, size, path):
    ...
    print("####################image_path : %s" % path)
    if not cv2.imwrite(os.path.normpath(path), images):
        raise Exception("Could not write image")
    return cv2.imwrite(os.path.normpath(path), images)
