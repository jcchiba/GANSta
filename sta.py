...
class StarGAN_v2() :
    def __init__(self, sess, args):
        self.model_name = 'gansta'
        self.sess = sess
        self.phase = args.phase
        self.checkpoint_dir = args.checkpoint_dir
        self.sample_dir = args.sample_dir
        self.sample_dir2 = args.sample_dir
        self.result_dir = args.result_dir
        self.log_dir = args.log_dir
        self.dataset_name = os.path.basename(args.dataset)
        self.dataset_path = os.path.join('', args.dataset)
        self.test_path = os.path.join('', args.dataset)
        ...
        print()
        print("####### FOLDERS ######")
        print("# sample : %s" % self.sample_dir)
        print("# checkpoint : %s" % self.checkpoint_dir)
        print("# log : %s" % self.log_dir)
        print("# result : %s" % self.result_dir)
    ...
    def gradient_panalty(self, real, fake, real_label, scope="discriminator"):
        ...
        grad = tf.gradients(logits, interpolated)[0] 
    ...
    def train(self):
        ...
        for idx in range(start_batch_id, self.iteration):
            ...
            mama = float(idx)/float(self.iteration)
            url = os.path.join(self.log_dir, self.dataset_name)
            file1 = open(os.path.join(self.log_dir, "log"), "w+")
            file1.write(str(mama))
            file1.close()
            if np.mod(idx+1, self.print_freq) == 0 :
                real_image = np.expand_dims(real_images[0], axis=0)
                save_images(real_image, [1, 1],'{}/real_{:07d}.jpg'.format(self.sample_dir, idx+1))
                merge_fake_x = None
                for ns in range(self.num_style) :
                    fake_img = np.transpose(fake_images[ns], axes=[1, 0, 2, 3, 4])[0]
                    if ns == 0 :
                        merge_fake_x = return_images(fake_img, [1, self.c_dim]) 
                    else :
                ...
                save_images(merge_fake_x, [1, 1],'{}/fake_{:07d}.jpg'.format(self.sample_dir, idx+1))
                url = os.path.join(self.sample_dir, 'fake_{:07d}.jpg'.format(idx+1))
                file1 = open(os.path.join(self.sample_dir2, "log"), "w+")
                file1.write(str(url))
                file1.close()
    ...
    def test(self):
        tf.global_variables_initializer().run()
        test_file_path = os.path.join(self.test_path, 'test')
        print(test_file_path)
        test_files = glob('{}/*.jpg'.format(test_file_path)) + glob('{}/*.png'.format(test_file_path))
        print("############TEST FILES")
        print(test_files)
        t_vars = tf.trainable_variables()
        ...
        index.write('<!DOCTYPE html><html lang="en" dir="ltr"><head><meta charset="utf-8"><title>Results Gansta</title></head><body><table><tr>')
        index.write("<th>name</th><th>input</th><th>output</th></tr>")
        for sample_file in tqdm(test_files):
            print("Processing image: " + sample_file)
            check_folder(self.result_dir + "/base")
            copy(sample_file, self.result_dir + "/base")
            namae = os.path.basename(sample_file)
            print(namae)
            ...
            for i in range(self.num_style) :
                ...
                if i == 0:
                    merge_x = return_images(fake_img, [1, self.c_dim]) 
                else :
                    x = return_images(fake_img, [1, self.c_dim])
            ...
            index.write("<td><img src='%s' width='%d' height='%d'></td>" % (self.result_dir + "/base/" + namae
    ...
    def refer_test(self):
        tf.global_variables_initializer().run()
        test_files = glob('{}/{}/*.jpg'.format(self.dataset_name, 'test')) + glob('.{}/{}/*.png'.format(self.dataset_name, 'test'))
        ...
        for sample_file in tqdm(test_files):
            print("Processing image: " + sample_file)
            sample_image = load_test_image(sample_file, self.img_width, self.img_height, self.img_ch)
            print("#############################SAMPLE IMAGE: %s" % sample_image)
            image_path = os.path.join(self.result_dir, '{}'.format(os.path.basename(sample_file)))
